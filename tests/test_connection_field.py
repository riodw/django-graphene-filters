...
def test_connection_field_custom_prefix():
    field = AdvancedDjangoFilterConnectionField(
        ConnNode, 
        filterset_class=ConnFilterSet, 
        filter_input_type_prefix="Custom"
    )
...
def test_resolve_queryset_invalid_form():
    ...
    with patch("graphene_django.filter.DjangoFilterConnectionField.resolve_queryset", return_value=ConnModel.objects.none()) as mock_super_resolve, \
         patch("django_graphene_filters.connection_field.tree_input_type_to_data", return_value={}):
