...
from django_graphene_filters.filters import SearchQueryFilter, SearchRankFilter, TrigramFilter
...
def test_create_search_query_data(filterset_class):
    ...
    with patch("django_graphene_filters.input_data_factories.create_search_vector") as mock_vector, \
...
def test_create_search_rank_data(filterset_class):
    ...
    input_type.get.side_effect = lambda k: {"weights": "mock_weights", "normalization": 1}.get(k)
...
def test_create_search_vector(filterset_class):
    ...
    input_type.get.side_effect = lambda k: {"config": "english", "weight": MagicMock(value="A")}.get(k)
...
def test_create_search_query():
    ...
    with patch("django_graphene_filters.input_data_factories.validate_search_query"), \
         patch("django_graphene_filters.input_data_factories.create_search_config") as mock_config, \
...
def test_create_search_query_complex_operations():
    ...
    def get_complex(k, d=None):
        if k == local_settings.AND_KEY: return [t1, t2]
        if k == local_settings.OR_KEY: return [t1, t2]
        if k == local_settings.NOT_KEY: return t1
    with patch("django_graphene_filters.input_data_factories.validate_search_query"), \
         patch("django_graphene_filters.input_data_factories.SearchQuery") as mock_query:
